...
export default function EmojiPictionary() {
  ...
  const gameRules = [
    "🎨 Guess the tech term represented by emojis",
    "💡 Use hints if you get stuck",
    "🤖 Get AI explanations to learn more",
    "🔄 Cycle through different puzzles",
    "🏆 Challenge yourself to guess without hints",
    "🧠 Expand your tech vocabulary"
  ];
  ...
  const fetchPuzzles = async () => {
    ...
    setApiError(null);
    try {
      const res = await axiosInstance.post("/breakzone/emoji-pictionary");
      setPuzzles(res.data);
      setCurrentIndex(0);
      setGuess("");
      setResult(null);
      ...
      setApiError(err.response?.data?.error || "Failed to load puzzles. Please try again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const checkAnswer = async () => {
    if (!currentPuzzle || !guess.trim()) return;
    
    setChecking(true);
    setApiError(null);
    try {
      const res = await axiosInstance.post("/breakzone/emoji-pictionary/answer", {
        guess,
        correctTerm: currentPuzzle.term,
      });
  ...
  const fetchAiExplanation = async () => {
    ...
    setApiError(null);
    try {
      const res = await axiosInstance.post("/breakzone/emoji-pictionary/ai-explaination", {
        term: currentPuzzle.term,
        context: "Explain this tech term in simple terms with examples"
      });
      setAiExplanation(res.data.explanation);
      setShowExplanation(true);
    } catch (err) {
      console.error("Error fetching explanation:", err);
      setApiError(err.response?.data?.error || "Failed to get explanation. Please try again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen min-w-screen  bg-gradient-to-br from-purple-900 to-pink-800 text-white p-6 flex flex-col items-center justify-start relative">
        ...
        >
          <motion.div whileHover={{ scale: 1.1 }} className="mr-2">
            <svg 
              ...
              className="h-5 w-5 group-hover:text-pink-400 transition-colors"
              viewBox="0 0 20 20" 
              fill="currentColor"
            >
              <path 
          ...
          <span className="font-medium text-sm group-hover:text-pink-400 transition-colors">
            Back to Break Zone
          </span>
        </motion.div>
      </Link>
      ...
      <motion.button
        ...
        whileTap={{ scale: 0.95 }}
        className="absolute top-4 right-34 bg-purple-800 hover:bg-purple-700 px-3 py-1 rounded-lg text-sm flex items-center"
      >
      ...
      >
        <motion.h1 
          className="text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-purple-300 to-pink-400 mb-2"
          initial={{ scale: 0.9 }}
          animate={{ scale: 1 }}
        ...
        <motion.p
          className="text-purple-200 mb-8 text-center max-w-md"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
        ...
        <AnimatePresence mode="wait">
          {loading ? (
            ...
            >
              ...
              <div className="space-y-4">
                <input
                  ...
                  className="w-full p-3 rounded-lg bg-white/90 text-purple-900 font-medium text-center text-lg shadow"
                ...
                <div className="flex flex-wrap justify-center gap-3">
                  ...
                  >
                    {showExplanation ? "Explanation Shown" : "AI Explanation"}
                  </motion.button>
                </div>
                ...
                {showHint && (
                  <motion.div
                    ...
                    animate={{ opacity: 1, y: 0 }}
                    className="p-3 bg-purple-800/50 rounded-lg text-purple-200 text-center"
                  >
                ...
                {showExplanation && aiExplanation && (
                  ...
                  >
                    <div className="flex items-center mb-2">
                      ...
                      <h3 className="text-lg font-bold text-blue-300">AI Explanation</h3>
                    </div>
            ...
            >
              <p className="text-gray-400">No puzzles available. Please try again later.</p>
              <button
